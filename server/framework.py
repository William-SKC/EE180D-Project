...
def localization_IMU(curr_state, lock, hist_loc, conn, direction, stride):
	...
	while 1:
		...
		movements = [0, 0, 0]
		[movements, cur_direction] = IMU_movement.IMU_movement(rec_data, cur_direction, stride)
		...
		with lock:
			...
			print('IMU: '+ cur_direction + ' ' + str(curr_state[1]) + ' ' + str(curr_state[2]))
			print('********************************\n')
		#time.sleep(0.5)
                with lock:
...
stride = ((float(height) * .413)/120)*1.5
...
p_IMU = partial(localization_IMU, curr_state, lock, hist_loc, conn, direction, stride)
